...
let isDeviceTestPassed = false;
let isCaptureTestSuccess = false;
let hasLocalAudioReport = false;
let playbackCompleteCheck = null;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isiPhone()) {
        ...
        if (playbackWaveContainer) {
            playbackWaveContainer.style.display = 'none';
        }
        if (playbackWaveLabel) {
            playbackWaveLabel.style.display = 'none';
        }
    }
            ...
            await new Promise((resolve, reject) => {
                const checkInterval = setInterval(() => {
                    ...
                    if (window.VERTC) {
                        ...
                        clearInterval(checkInterval);
                        reject(new Error('SDK加载超时，请检查网络连接或CDN地址'));
...
function getBrowserInfo() {
    ...
    if (ua.indexOf('MicroMessenger') > -1) {
        ...
        browserName = '夸克浏览器';
        const match = ua.match(/Quark\/([\d.]+)/);
        if (match) browserVersion = match[1];
    } else if (ua.indexOf('UCWeb') > -1 || ua.indexOf('UCBrowser') > -1) {
        browserName = 'UC浏览器';
        const match = ua.match(/UC(?:Browser)?\/([\d.]+)/);
        if (match) browserVersion = match[1];
    } else if (ua.indexOf('QQBrowser') > -1 || ua.indexOf('MQQBrowser') > -1) {
        browserName = 'QQ浏览器';
...
function updateWaveBars(containerId, volume) {
    ...
    // 如果是 iPhone 设备且是播放音量波形，不更新
    if (isiPhone() && containerId === 'playback-wave-container') {
        return;
    }
...
async function startAudioCapture() {
    try {
        ...
        log('初始化状态变量: isCaptureTestSuccess=false, hasLocalAudioReport=false', 'info');
        isCaptureTestSuccess = false;
        hasLocalAudioReport = false;
        isCaptureFailed = false;
        ...
        try {
            ...
            log('设置 isCaptureTestSuccess = true', 'info');
            isCaptureTestSuccess = true;
        } catch (error) {
...
async function stopAudioCaptureAndPlay() {
    try {
        ...
        let micStatus = '异常';
        let micStatusType = 'error';
        
        // 调试日志
        log(`麦克风判定 - 前置设备检测: ${isDeviceTestPassed}, startAudioDeviceRecordTest成功: ${isCaptureTestSuccess}, hasLocalAudioReport: ${hasLocalAudioReport}, isCaptureFailed: ${isCaptureFailed}`, 'info');
        
        // 判定麦克风状态：如果采集失败，直接异常；否则只要前置设备检测成功，且onLocalAudioPropertiesReport有数值变化就认为成功
        if (isCaptureFailed) {
            micStatus = '异常';
            micStatusType = 'error';
        } else if (isDeviceTestPassed && hasLocalAudioReport) {
            micStatus = '成功';
        ...
        if (!isiPhone()) {
            ...
            // iPhone 设备只根据麦克风状态设置 section 图标
            if (micStatus === '成功') {
                setSectionIcon('audio', 'success');
            } else {
                setSectionIcon('audio', 'error');
...
async function checkSpeakerPlayback(deviceTestPassed = true) {
    console.log('=== 音频检测开始 ===');
    log('开始音频检测准备...', 'info');
...
async function startDetection() {
    ...
    try {
        ...
        const deviceTestPassed = await checkDeviceCapability();
        await new Promise(resolve => setTimeout(resolve, 500));
        
        await checkVideoCodecs();
        await new Promise(resolve => setTimeout(resolve, 500));
        
        await checkSpeakerPlayback(deviceTestPassed);
        await new Promise(resolve => setTimeout(resolve, 500));
